    ...
    def __sub__(self, other):
        if isinstance(other, Cell):
            ...
            if dif > 0:
                ...
                return "Ошибка! Операция вычитания невозможна, так как разность количества ячеек двух клеток должна быть больше нуля"
        else:
            raise TypeError
...
assert (cell_1 + cell_2).quantity == 23
assert (cell_1 - cell_2).quantity == 7
assert (cell_1 * cell_2).quantity == 120
assert (cell_1 / cell_2).quantity == 1
